...
thresholds = np.append(thresholds, 1) # Align arrays

target_recall = 0.80
# Find the first index where recall is >= target_recall
idx_for_80_recall = np.argmax(recall >= target_recall)
threshold_for_80_recall = thresholds[idx_for_80_recall]
precision_for_80_recall = precision[idx_for_80_recall]

print(f"Using our champion model:")
print(f"To achieve a recall of at least {target_recall*100:.0f}%, you should use a probability threshold of: {threshold_for_80_recall:.4f}")
print(f"At this threshold, the precision will be {precision_for_80_recall:.4f}. This means {precision_for_80_recall*100:.1f}% of the claims you flag will actually be fraudulent.")
